...
class Sk_Bike_Booking_Admin {
    ...
    public function __construct( $plugin_name, $version ) {
        ...
        if ( isset( $_POST['action'] ) && $_POST['action'] === 'bb_export_bookings' ) {
			$period = false;
			if( !empty( $_POST['bb_export_period'] ) ){
				$period = $_POST['bb_export_period'];
			}

			$results = $this->get_bookings( $period );
    ...
    public function export_page_view() {
        ...
        global $wpdb;
        $periods = $wpdb->get_col( "SELECT DISTINCT meta_value FROM $wpdb->postmeta WHERE meta_key = 'bb-period'" );
        rsort( $periods );

        ?>
		<div class="wrap">
			<h1><?php _e( 'Exportera', 'bikebooking_textdomain' );?></h1>
			<form method="post">
				<table class="form-table">
					<tbody>
						<tr>
							<th scope="row"><label for="bb-export-period"><?php _e( 'Välj period', 'bikebooking_textdomain' );?></label></th>
							<td>
								<select name="bb_export_period" id="bb-export-period">
									<option value=""><?php _e( 'Samtliga bokningar', 'bikebooking_textdomain' );?></option>
				...
				<input type="hidden" name="action" value="bb_export_bookings">
				<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="Exportera till Excel"></p>
			</form>
		</div>
	 ...
	 * @return void
	 */
    public function get_bookings( $period = false ) {
        ...
        foreach ( $posts as $post ) {
            $hash                = get_post_meta( $post->ID, 'bb-hash', true );
            $bookings[$post->ID] = explode( ':', base64_decode( $hash ) );
        }

        $i = 0;
        foreach ( $bookings as $key => $booking ) {
            $data[$i]['ID']     = $key;
            $data[$i]['name']   = $booking[5];
            $data[$i]['email']  = $booking[0];
            $data[$i]['period'] = $booking[2] . ':' . $booking[3];
            $i++;
        }
     ...
     * @since 1.0.0
     */
    public function export_to_excel( $results ) {
        $time     = date( 'Ymd_His' );
        $filename = 'cykelbokningar_' . $time . '.xls';
        ...
        foreach ( $results as $row ) {
            ...
            if ( !$flag ) {
                ...
                echo "" . implode( "\t", array_values( $caption ) ) . "\r\n";
                $flag = true;
    ...
    public function clean_data_for_excel( &$str ) {
        //$str = iconv('UTF-8', 'ISO-8859-1', htmlspecialchars_decode( $str ) );
        $str = iconv( 'UTF-8', 'ISO-8859-1//IGNORE', htmlspecialchars_decode( $str ) );
