...
class Sk_Bike_Booking_Public {
    ...
    public function book_bike() {
        ...
        $booker['compliance'] = $_POST['booker_compliance'];
        ...
        if ( empty( $booker['email'] ) || empty( $booker['name'] ) || empty( $booker['phone'] ) ) {
            ...
            return wp_send_json( $response );
        } elseif ( $booker['compliance'] === 'false' ) {
            $response['error'] = 'Du behöver godkänna att vi lagrar de uppgifter du anger i bokningsformuläret.';
            return wp_send_json( $response );
        } else {
